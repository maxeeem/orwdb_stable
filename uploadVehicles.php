...
function getFile() { global $pn, $vehicles;
	...
	while ($row = fgetcsv($file)) {
		...
		if (validate($i, $row[1], $row[2]) !== '') $err[] = validate($i, $row[1], $row[2]);
	
		$i++;
	
		$pn[$row[0]]['Applications'][] = array('Make' => $row[3],
																					 'Model' => $row[4],
																					 'Start year' => $row[1],
																					 'End year' => $row[2],
																					 'Extra' => $row[5]);
	...
	while (count($arr) > 0) {
		...
		if (count($arr) > 1) {
			...
			if ($arr[1] !== ($arr[0] + 1)) $res[] = array_shift($arr);
				...
				while (count($arr) > 1 && $arr[1] == ($arr[0] + 1)) array_shift($arr);
...
function check($vehicles) { global $validVehicles;
	...
	foreach (array_keys($vehicles) as $make) {
	
		if (array_key_exists($make, $validVehicles)) {
			...
			foreach (array_keys($vehicles[$make]) as $model) {
				...
				if (array_key_exists($model, $validVehicles[$make])) { $res = $vehicles[$make][$model]['Years'][0];
					...
					foreach ($vehicles[$make][$model]['Years'] as $y => $years) {
					
						if (array_key_exists(($y + 1), $vehicles[$make][$model]['Years'])) {
	...
	if (isset($addVehicles)) {

		echo "<h2>Vehicles not in ORW Database</h2>";
		
		ksort($addVehicles);
	
		foreach (array_keys($addVehicles) as $addmake) {
			...
			foreach (array_keys($addVehicles[$addmake]) as $addmodel) {
				...
				echo "<strong>$addmodel</strong><br />";
				
				$res = $addVehicles[$addmake][$addmodel]['Years'][0];
				
				foreach ($addVehicles[$addmake][$addmodel]['Years'] as $y => $years) {
					
					if (array_key_exists(($y + 1), $addVehicles[$addmake][$addmodel]['Years'])) {
